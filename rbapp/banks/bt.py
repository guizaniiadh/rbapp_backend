...
#             amount = credit - debit
#             # Automatically set the bank from the ledger_entry's agency
#             bank = ledger_entry.agency.bank
#             transactions.append({
#                 'bank_ledger_entry_id': ledger_entry.id,  # Use the ID, not the object
#                 'bank_id': bank.code,  # Use the code, not id
...
# def match_customer_bank_transactions(filtered_bank_transactions, customer_transactions, output_dir, payment_identifications=None):
#     """
...
#     # Check if any payment identification is grouped
#     grouped_mode = False
#     if payment_identifications:
#         grouped_mode = any(pid.get('grouped', False) for pid in payment_identifications)
...
#                 # Use total_amount if grouped, else use amount
#                 if grouped_mode:
#                     score_amount = score_amount_exact(customer_row['total_amount'], bank_row['amount'])
#                 else:
#                     score_amount = score_amount_exact(customer_row['amount'], bank_row['amount'])
#                 # Only include score_reference if not grouped
#                 if not grouped_mode:
...
#                 # Adjust total_score calculation
#                 if grouped_mode:
#                     total_score = 0.25 * score_date + 0.6 * score_amount
#                 else:
#                     total_score = 0.25 * score_date + 0.6 * score_amount + 0.15 * score_reference
...
#                     try:
#                         from rbapp.models import CustomerTransaction, BankTransaction
#                         ct = CustomerTransaction.objects.get(id=customer_transaction_id)
#                         bt = BankTransaction.objects.get(id=matched_bank_id)
#                         matching_customer_transactions = CustomerTransaction.objects.filter(
...
#                             matching_ct.save()
#                     except (CustomerTransaction.DoesNotExist, BankTransaction.DoesNotExist):
#                         continue
...
# # Example: Add this after parsing in your view or function
# from rbapp.models import BankTransaction, BankLedgerEntry, Bank

# # Do NOT leave this at the module/global level!
